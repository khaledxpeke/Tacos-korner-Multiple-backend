...
exports.getRestaurants = async (req, res) => {
    try {
      ...
      if (req.user.user.role === "admin"|| req.user.user.role === "client") {
        restaurants = await Restaurant.find().select('name description active createdAt');
      } else {
        ...
        }).select('name description active createdAt');
      }
  ...
  exports.deleteRestaurant = async (req, res) => {
    try {
      ...
      if (restaurant.settings) {
        await Settings.findByIdAndDelete(restaurant.settings);
      }
    ...
    try {
      ...
      if (req.body.users && Array.isArray(req.body.users)) {
        ...
        return res.status(400).json({ 
          message: "Invalid request format. Provide either userId+role or an array of users."
        });
      }
      ...
      for (const { userId, role } of usersToProcess) {
        try {
          ...
          if (role === "waiter") {
            ...
            const hasRestaurant = user.restaurants && user.restaurants.some(
  ...
  exports.removeUserFromRestaurant = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: "User removed from restaurant successfully" });
    } catch (error) {
      res.status(500).json({ message: error.message });
    }
  };